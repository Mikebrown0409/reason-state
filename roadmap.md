...
- [~] 5) tools (real only): `xSearch` uses real X (fail loud); booking real-ish (Stripe-test + calendar clash + governance gate); Grok 4.1 only; remaining: full live-agent run with X + governance DAG and persisted log.
...
- [~] 8) tests: determinism + Grok validation + node-shape validation + context-builder budget/determinism + simple/X agent smoke added; still need full integration suite (Tokyo→retract→Amsterdam replay with hotel/flight assertions), live xSearch+Grok calls, governance gating, token-savings assertions.
