...
- [~] LLM-driven patching: Grok 4.1 with deterministic context builder (summaries-only), strict system prompt, validated patch parsing/backoff, engine-assigned IDs (no LLM IDs); remaining: richer node-shape validation and multi-turn governance coverage
...
- [~] 2b) LLM orchestration: Grok 4.1 with strict prompt; deterministic context builder (token-budgeted, summaries-only) feeds Grok; parse/validate/backoff of model patches; engine assigns IDs. Remaining: node-shape validation, integration tests, and multi-turn governance interplay.
...
- [~] 5) tools (real only): `xSearch` uses real X (fail loud); booking real-ish (Stripe-test + calendar clash + governance gate); Grok 4.1 only; remaining: full live-agent run with X + governance DAG.
...
- [~] 8) tests: determinism + Grok validation + simple agent smoke added; still need full integration suite (Tokyo→retract→Amsterdam replay with hotel/flight assertions), live xSearch+Grok calls, context-builder budget tests, governance gating, token-savings assertions.
- [~] 9) polish/docs/build: README env + .env.example + “what’s verified” section added; still need demo GIF placeholder, LOC audit, build/publish path, and tool fallback notes.
