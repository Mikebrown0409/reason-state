...
- [~] 3) reconciliation & governance: dirty propagation via adjacency + contradiction edges added; depends_on blocking and contradiction detection keep nodes dirty; canExecute gates unknown/dirty/invalid assumptions; remaining: richer adjacency/timestamps and expanded self-heal loop beyond current coverage.
...
- [~] 8) tests: determinism + Grok validation + node-shape validation + context-builder budget/determinism + reconciliation edges + simple/X agent smokes added; integration replay harness added (simple agent, log replay, booking blocked). Still need full integration suite (Tokyo→retract→Amsterdam replay with hotel/flight assertions), governance gating assertions, token-savings assertions.
